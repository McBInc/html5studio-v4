...
import { Database, Radio, RefreshCw, AlertTriangle, Download } from "lucide-react";
...
export default function StudioDirectory() {
  ...
  const refetch = async () => {
    try {
      const res = await fetch("/api/agents/compileDirectory");
      if (!res.ok) throw new Error("V4 Backend Connection Failed");
  ...
  const handleSeed = async () => {
    setSeeding(true);
    setSeedResult(null);
    fetch("/api/agents/compileDirectory", { method: "POST" })
      .then((res) => {
        if (!res.ok) throw new Error(`V4 Backend 500: Server Error`);
        return res.json();
      })
      .then((data) => {
        setSeedResult(data);
  ...
  const exportToCSV = () => {
    const headers = ["Studio Name", "Website", "Contact Email", "Country", "Size", "Platforms", "Games", "Risk Level", "Compliance Risks", "Tags"];
    const rows = studios.map(s => [
      `"${s.name || ''}"`, `"${s.website || ''}"`, `"${s.contact_email || ''}"`, `"${s.country || ''}"`,
      `"${s.studio_size || ''}"`, `"${(s.platforms || []).join(", ")}"`, `"${(s.games || []).join(", ")}"`,
      `"${(s.risk_level || '').toUpperCase()}"`, `"${(s.compliance_risks || []).join(" | ")}"`, `"${(s.tags || []).join(", ")}"`
    ]);
    const csvContent = [headers.join(","), ...rows.map(r => r.join(","))].join("\n");
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.setAttribute("download", `Studio_Intelligence_Records_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const filteredStudios = useMemo(() => {
    return studios.filter((s) => {
      ...
      if (q && !(
        ...
        s.games?.some((g: string) => g.toLowerCase().includes(q)) ||
        s.platforms?.some((p: string) => p.toLowerCase().includes(q)) ||
      ...
      <section className="relative pt-32 pb-16 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6">
          ...
          >
            ...
            <h1 className="text-4xl sm:text-5xl font-black tracking-tight mb-4 uppercase italic">
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={exportToCSV}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-mono bg-primary/10 border border-primary/20 text-primary hover:bg-primary/20 transition-all font-bold uppercase tracking-widest italic"
              >
              ...
              <button
                onClick={handleSeed}
                disabled={seeding}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-mono border border-white/10 text-muted-foreground hover:text-foreground hover:border-white/20 transition-all disabled:opacity-40 uppercase font-black"
              >
                <RefreshCw className={`w-3 h-3 ${seeding ? "animate-spin" : ""}`} />
                {seeding ? "Researching..." : "Run Intelligence Agent"}
              </button>
            </div>
          </div>
            ...
            >
              {seedResult.error
                ...
                : seedResult.background
                  ? `⚡ Agent is running in the background — studios are updating live.`
                  : `✓ Agent complete — ${seedResult.studios_researched} researched, ${seedResult.upserted} upserted`}
            </div>
          )}
          ...
          {isLoading ? (
            ...
            <div className="text-center py-24">
              ...
              <p className="text-muted-foreground/50 text-sm font-mono uppercase font-black">
                {studios.length === 0
                  ? 'No studios indexed yet.'
                  : "No studios match parameters."}
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
