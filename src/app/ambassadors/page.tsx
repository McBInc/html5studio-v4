...
// ─── LOCAL COMPONENTS (VERBATIM V5) ───────────────────────────────────────────

function StatCard({ disruption }: { disruption: any }) {
  return (
    <motion.div
...
function InvestorRow({ investor, index }: { investor: any, index: number }) {
  return (
    <motion.tr
...
export default function AmbassadorProgramPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-inter selection:bg-blue-500/30 overflow-x-hidden">
      <NewsTicker />
      <Navbar />
      ...
      <section id="how-it-works" className="py-20 border-t border-white/5">
        <div className="max-w-5xl mx-auto px-6">
          <div className="text-center mb-12">
            ...
            <p className="text-muted-foreground text-lg italic">Six stages. Zero cost to you. A transformation every investor can see.</p>
          </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-8">
            {JOURNEY_STAGES.map((stage, i) => (
              <button key={i} onClick={() => setActiveStage(i)}
                ...
                <span className="text-[9px] font-mono text-center leading-tight uppercase font-black" style={{ color: activeStage === i ? stage.color : "#444" }}>
                  {stage.label}
                </span>
              </button>
            ))}
          ...
          <AnimatePresence mode="wait">
            <motion.div key={activeStage} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
              ...
              <h3 className="text-2xl font-black mb-3 italic uppercase" style={{ color: JOURNEY_STAGES[activeStage].color }}>
                Stage {activeStage + 1}: {JOURNEY_STAGES[activeStage].label}
              </h3>
              <p className="text-lg text-muted-foreground max-w-xl mx-auto italic leading-relaxed">{JOURNEY_STAGES[activeStage].desc}</p>
            </motion.div>
          </AnimatePresence>
        </div>
      </section>
      ...
      <section className="py-20 border-t border-white/5">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl font-black tracking-tight mb-3 italic uppercase">The Trustless Revenue Engine</h2>
          ...
          <div className="rounded-2xl border p-6 mb-10 text-center"
            ...
            <p className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground/50 mb-4 italic">Live Transaction Flow</p>
            <div className="flex flex-wrap items-center justify-center gap-3 text-sm font-mono">
              <div className="px-4 py-2 rounded-lg border border-white/15 bg-white/5 text-foreground font-bold">Player Payment<br /><span className="text-xs text-muted-foreground uppercase">(Stars / TON / IAP)</span></div>
              ...
              <div className="px-4 py-2 rounded-lg border font-bold" style={{ borderColor: "rgba(155,89,182,0.4)", color: "#9B59B6", background: "rgba(155,89,182,0.08)" }}>
                TON Smart Contract<br /><span className="text-xs opacity-60 uppercase italic">Atomic Split</span>
              </div>
              <ArrowRight className="w-5 h-5 text-muted-foreground/40" />
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {TON_MECHANICS.map((m, i) => {
              const Icon = m.icon;
              return (
                <div key={m.title} className="rounded-xl border p-5" style={{ borderColor: "rgba(155,89,182,0.15)", background: "rgba(155,89,182,0.04)" }}>
                  <Icon className="w-5 h-5 mb-3 text-purple-500" />
                  <h3 className="text-sm font-black text-foreground mb-2 italic uppercase">{m.title}</h3>
                  <p className="text-xs text-muted-foreground leading-relaxed italic">{m.desc}</p>
                </div>
              );
            })}
          </div>
      ...
      <section className="py-20 border-t border-white/5">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-10">
            <h2 className="text-3xl font-black tracking-tight mb-3 italic uppercase text-white">Target Investor Landscape — 2026</h2>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto font-black uppercase text-[10px] tracking-widest opacity-60">$6.2B+ AUM // DIP Certified Registry</p>
          </div>

          <div className="rounded-2xl border border-white/8 overflow-hidden bg-black shadow-3xl">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/8 bg-white/[0.02]">
                    {["Fund", "Focus", "Check Size", "DIP Status", "Why They Care"].map(h => (
                      <th key={h} className="text-left px-4 py-8 text-[10px] font-black font-mono uppercase tracking-[0.4em] text-gray-700 italic">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
      ...
      <section className="py-20 border-t border-white/5">
        <div className="max-w-3xl mx-auto px-6">
          <h2 className="text-3xl font-black mb-12 text-center uppercase italic text-white">Protocol FAQs</h2>
      ...
      {/* CTA */}
      <section id="apply" className="py-24 border-t border-white/5">
        <div className="max-w-3xl mx-auto px-6 text-center">
          <div className="rounded-[48px] border p-12 relative overflow-hidden bg-[#05050a] shadow-3xl"
            style={{ borderColor: "rgba(30,111,240,0.25)" }}>
            <Award className="w-16 h-16 mx-auto mb-6 text-blue-500" />
            <h2 className="text-4xl font-black mb-4 uppercase italic">Tell Us Your Story</h2>
            <p className="text-gray-400 text-lg mb-8 italic max-w-lg mx-auto leading-relaxed">
              Where are you right now? Platform rejection? SDK nightmare? Build that won't load? 
              The more real your story, the more powerful your Ambassador arc.
            </p>
            <button onClick={() => setAgentOpen(true)}
              className="px-12 py-6 rounded-3xl bg-gradient-to-r from-blue-600 to-red-600 hover:from-blue-500 hover:to-red-500 text-white font-black text-xl uppercase tracking-tighter transition-all flex items-center justify-center gap-4 mx-auto shadow-2xl shadow-blue-500/20">
