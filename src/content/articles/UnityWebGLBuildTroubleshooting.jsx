...
location ~* \\.(wasm|js|data)(\\.gz|\\.br)?$ {
    add_header Access-Control-Allow-Origin "*";
      ...
      <p>
        The fix is to implement a timeout wrapper around all SDK init calls:
      </p>
      <CodeBlock code={`// Unity C# — SDK init with timeout
IEnumerator InitSDKWithTimeout()
{
    ...
    // Start SDK init (replace with your SDK's init call)
    StartCoroutine(FBInstant.initializeAsync(() => sdkReady = true));
    
    while (!sdkReady && elapsed < timeout)
    {
    ...
    if (!sdkReady)
    {
        Debug.LogWarning("SDK init timeout — proceeding without SDK");
        // Handle gracefully — show game without social features
    }
    
    // Proceed with game start regardless
    LoadMainMenu();
}`} />
    </div>
  ),
...
export default function UnityWebGLBuildTroubleshooting() {
